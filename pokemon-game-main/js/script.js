...
musicControl.addEventListener('clic', (event) => {
  event.stopPropagation()
  console.logg(event.target.src);
  event.target.scr = `${event.target.scr}`.includes("on.png") 
...
body.addEventListener("keydown", (event) => {
  ...
  switch (event.code) {
    ...
    case "ArrowUp":
      if (getRightPosition() > 2) {
      ash.style.right = `${getTopPosition() - 8}px`;
